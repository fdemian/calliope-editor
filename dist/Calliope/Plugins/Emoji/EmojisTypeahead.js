 ...
 */
function getEmojiOffset(documentText, entryText, offset) {
 ...
 */
function createEmojiNodeFromSearchResult(editor, selectedEntry, match) {
    editor.update(() => {
        ...
        const mentionOffset = getEmojiOffset(textContent, selectedEntry, characterOffset);
...
const EmojisTypeahead = ({ close, editor, resolution, results, config, shortList }) => {
    ...
    return (_jsx("div", { "aria-label": "Suggested mentions", id: "mentions-typeahead", ref: divRef, role: "listbox", children: _jsx("ul", { children: results.slice(0, EMOJI_SUGGESTIONS_LENGTH_LIMIT).map((result, i) => (_jsx(EmojisTypeaheadItem, { index: i, isSelected: i === selectedIndex, onClick: () => {
                    setSelectedIndex(i);
                    applyCurrentSelected();
                }, onMouseEnter: () => {
                    setSelectedIndex(i);
                    ...
                    setSelectedIndex(null);
                }, result: result }, result))) }, "mentions-typeahead-list") }));
};
